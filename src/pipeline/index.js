...
function summarizeUpdates(changes) {
  ...
  for (const change of changes) {
    ...
    for (const dep of allDeps) {
      ...
      summary.total++;
      if (dep.updateType === 'major') summary.major++;
      else if (dep.updateType === 'minor') summary.minor++;
      else if (dep.updateType === 'patch') summary.patch++;
...
export async function runPhaseCollect(repositories) {
  ...
  const spinner = ora({
    text: `Coletando dados: 0/${repositories.length}`,
    spinner: 'dots',
  }).start();
